 ...
 * DATABASE: PostgreSQL
 * AUTHOR: trandnquang
 * VERSION: 1.2 (Final Backend Release)
 * * NOTES:
 ...
 * 4. Architecture:
 * - 'Service' entity renamed to 'Utility'.
 * - Implemented Price Locking (Snapshot) in 'contract_utility'.
 * - Generated Columns removed in favor of Application-side calculation (Java BigDecimal).
 * ======================================================================================
 */
...
-- Renamed from 'service' to 'utility'
...
-- Includes PRICE LOCKING features (Snapshot)
CREATE TABLE contract_utility (
    ...
    -- [SNAPSHOT - CRITICAL] Locked Price & Tier Config at the time of signing
    -- Ensures that future price changes in 'utility' table do not affect existing contracts
    locked_price NUMERIC(15, 2),
    locked_tier_config JSONB,

    -- Audit
    -- Added is_deleted to match Java Entity
    is_deleted BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    created_by VARCHAR(50),
...
CREATE TABLE invoice (
    ...
    -- Removed Generated Column due to Hibernate 6 compatibility issues.
    -- This will be calculated by Application Logic.
...
CREATE TABLE invoice_detail (
    ...
    -- Removed Generated Column. Application will set (new - old).
    usage_amount NUMERIC(10, 2) DEFAULT 0,
...
CREATE TRIGGER update_utility_modtime BEFORE UPDATE ON utility FOR EACH ROW EXECUTE PROCEDURE update_timestamp();
CREATE TRIGGER update_contract_modtime BEFORE UPDATE ON contract FOR EACH ROW EXECUTE PROCEDURE update_timestamp();
CREATE TRIGGER update_contract_utility_modtime BEFORE UPDATE ON contract_utility FOR EACH ROW EXECUTE PROCEDURE update_timestamp();
